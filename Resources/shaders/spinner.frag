...
{
	return mix(a, b, clamp((value + (1.0 / smoothness)) * smoothness, 0.0, 1.0));
}
...
{	
	...
	vec2 centeredCoords = (v_texcoords - 0.5) * 2.0;

	if (centeredCoords == vec2(0.0))
		discard;
	vec2 normalizedCoords = normalize(centeredCoords);

	float dist = length(centeredCoords);

	float opacity = smoothSelect(0.0, smoothSelect(1.0, 0.0, dist - 1.0, 20.0), dist - 0.75, 20.0);

	if (opacity < 0.0)
		discard;

	float spinTime = mod(u_time * 2.0, 2.0);
	float angle;
	if (spinTime < 1.0)
	{
	...
	float value = smoothSelect(1.0, 0.0, (angle - spinTime - abs(sin(u_time * 1.5) / 2.0)) - 0.5, 50.0);
	value *= smoothSelect(0.0, 1.0, angle - spinTime - 0.01, 10.0);

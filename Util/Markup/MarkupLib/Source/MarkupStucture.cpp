...
std::string MarkupElement::WriteCode(ParseResult& MarkupElements)
{
	...
	{
		Out << "\t\t" << Format::FormatString(i.Name, { Format::FormatArg("val",
...
static ConvertInfo ConvertValue(UIElement* Target, UIElement* Root, kui::stringParse::StringToken Value,
	VariableType Type, std::string ValueName, ParseResult& MarkupElements)
{
	ConvertInfo Out;
	...
	if (Out.Variable || Out.IsTranslated || Out.ValueGlobal)
	{
		if (Out.Variable && Out.Variable->Type != Type)
		{
			...
			{
				kui::parseError::ErrorNoLine("Variable '" + Out.Variable->Token.Text +
					"' does not have the correct type for the value of '" + ValueName + "'");
			}
		}
...
static std::string WriteElementProperty(UIElement* Target, UIElement* Root,
	std::string ElementName, Property p, const PropertyElement& i, ParseResult& MarkupElements)
{
	using namespace kui::stringParse;
	...
	for (auto& FormatString : i.SetFormat)
	{
		...
		else if (i.VarType == VariableType::Size || i.VarType == VariableType::SizeNumber)
		{
			...
			SetValue = val.ToCppCode(i.VarType == VariableType::Size);
		}
		else
		{
...
std::string UIElement::MakeCode(std::string Parent, UIElement* Root, size_t& Depth, ParseResult& MarkupElements)
{
	...
	if (!Parent.empty())
	{
		...
		OutStream << ElemName << " = new " << ClassName
			<< "(" << DefaultConstructors[GetTypeFromString(TypeName)] << ");\n";
	}
	else
	{
		ElemName = "this";
	...
	for (auto& elem : MarkupElements.Elements)
	{
		...
		for (auto& prop : PropertyCopy)
		{
			...
			ConvertInfo Result = ConvertValue(this, Root, prop.Value, Variable->second.Type,
				Variable->second.Token, MarkupElements);
			...
			else if (Variable->second.Type == VariableType::Size || Variable->second.Type == VariableType::SizeNumber)
			{
				auto val = Size(Result.Value);
				Result.Value.Text = val.ToCppCode(Variable->second.Type == VariableType::Size);
			}

			OutStream << "\t\t" << Format::FormatString(SetFormat, {Format::FormatArg("val",
				ToCppCode(Result.Value))}) << ";" << std::endl;
